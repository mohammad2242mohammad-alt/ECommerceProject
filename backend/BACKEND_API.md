...
```text
Admin:    09120000001 / password
Customer: 09120000002 / password
...
Cart items use `variant_id`. The legacy `product_variant_id` request field is
accepted temporarily for clients from the earlier branch.
...
The following is the required flow to execute locally before merging. It is a
checklist, not a claim that CI has already run it:
