...
import '../../../data/models/category_model.dart' as category_model;
...
class CategoryScreen extends ConsumerWidget {
  ...
  category_model.Category? _findCategory(
    ...
    if (kIsWeb) {
      ...
      if (!opened && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('باز کردن صفحه زیر‌دسته ممکن نشد'),
          ),
        );
      }
    ...
    return Scaffold(
      ...
      body: categoriesAsync.when(
        ...
        error: (error, stackTrace) {
          debugPrint(
            'CATEGORY SCREEN ERROR: $error',
          );

          debugPrint(
            'CATEGORY SCREEN STACK: $stackTrace',
          );
        ...
        data: (categories) {
          ...
          return RefreshIndicator(
            onRefresh: () async {
              ...
              ref.invalidate(
                popularProductsByCategoryProvider(
                  categoryId,
                ),
              );
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      productsAsync.when(
                        ...
                        error: (error, stackTrace) {
                          debugPrint(
                            'POPULAR PRODUCTS ERROR: $error',
                          );

                          debugPrint(
                            'POPULAR PRODUCTS STACK: '
                            '$stackTrace',
                          );

                          return AppError(
                            message:
                                'خطا در دریافت محصولات محبوب',
                            onRetry: () {
                              ref.invalidate(
                                popularProductsByCategoryProvider(
                                  categoryId,
                        ...
                        data: (products) {
                          if (products.isEmpty) {
                            return const AppEmpty(
                              message:
                                  'در این دسته هنوز محصولی وجود ندارد',
                              icon:
